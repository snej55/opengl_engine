...
FontRenderer::FontRenderer(EngineObject* parent)
    : EngineObject{"FontRenderer", parent}
{
}
...
bool FontRenderer::init(const std::string& fontPath, const int height)
{
    ...
    for (unsigned char c{0}; c < 128; c++)
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, static_cast<GLsizei>(m_face->glyph->bitmap.width), static_cast<GLsizei>(m_face->glyph->bitmap.rows), 0, GL_RED, GL_UNSIGNED_BYTE, m_face->glyph->bitmap.buffer);
        // texture options
...
void FontRenderer::renderText(const Shader& shader, const std::string& text, float x, const float y, const float scale, const glm::vec3&& color)
{
